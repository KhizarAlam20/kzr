...
const HeroSection = ({height, hoveredDiv, handleMouseEnter, handleMouseLeave, getBackgroundColor, getTextColor }) => {
  ...
  return (
    <div
    className={` style-links col-span-6  select-none px-10 sm:pt-12 xl:pt-0 xl:py-4  flex flex-col justify-end  md:text-left rounded-[${rounded}] h-[37vh] hover:scale-95  transition-all  duration-1000 ease-in-out  hover:shadow-lg ${isLightTheme ? 'bg-backGray' : 'bg-jet'} ${isLightTheme ? 'text-f11' : 'text-white'}` }
      style={{
    ...
    >
      ...
      <p className="font-extralight mb-8 sm:text-left sm:mb-8 xl:mb-4" style={{ fontFamily: "Syne, sans-serif" }}>
        <span className="italic text-5xl lg:text-5xl font-medium text-block "> A </span>
        <strong className="color-underline text-5xl lg:text-5xl font-medium">Software </strong>
        <br/>
        <span className=" text-5xl lg:text-5xl font-medium"> Engineer</span>
        ...
        <span className="text-xl lg:text-[16px]  text-block font-poppins font-light">
          2.5D Animations | Logo Design | Photography</span>
      </p>
